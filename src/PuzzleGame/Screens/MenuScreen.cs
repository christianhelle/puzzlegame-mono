...
public abstract class MenuScreen : GameScreen
{
    ...
    protected virtual string FooterText
        => "Arrow keys, mouse wheel, or Tab to move | Enter or click to choose | Esc or right-click to go back";
    ...
    protected virtual Rectangle GetMenuEntryHitBounds(MenuEntry entry)
    {
        ...
        return new(
            (int)entry.Position.X - horizontalPadding,
            (int)(entry.Position.Y - entry.GetHeight(this) / 2f - padding),
            width,
            height);
    }
    ...
    public override void HandleInput(InputState input)
    {
        ...
        if (input.IsMenuSelect(ControllingPlayer, out var playerIndex))
            OnSelectEntry(selectedEntry, playerIndex);
              ...
              && hoveredEntryIndex.HasValue) OnSelectEntry(hoveredEntryIndex.Value, playerIndex);
        ...
        bool coveredByOtherScreen)
    {
        ...
        if (menuEntries.Count == 0) return;
        selectedEntry = Math.Clamp(selectedEntry, min: 0, menuEntries.Count - 1);
    ...
    public override void Draw(GameTime gameTime)
    {
        ...
        var safeArea = ScreenManager.GetSafeArea(
            horizontalMarginRatio: 0.12f,
            verticalMarginRatio: 0.12f);
        var panelBounds = GetMenuPanelBounds(safeArea);
        var titleScale = MathHelper.Clamp(TitleScale * LayoutScale, min: 1.15f, max: 1.55f);
        var transitionOffset = (float)Math.Pow(TransitionPosition, y: 2);
    ...
    protected virtual void UpdateMenuEntryLocations()
    {
        ...
        var availableHeight = Math.Max(val1: 0f, contentBottom - contentTop);
        var startY = contentTop + MathF.Max(x: 0f, (availableHeight - totalMenuHeight) / 2f);
        var position = new Vector2(x: 0f, startY + menuEntries[0].GetHeight(this) / 2f);
        foreach (var menuEntry in menuEntries)
        {
            position.X = panelBounds.Center.X - menuEntry.GetWidth(this) / 2f;
            if (ScreenState == ScreenState.TransitionOn)
                position.X -= transitionOffset * (210f * LayoutScale);
    ...
    private Rectangle GetMenuPanelBounds(Rectangle safeArea)
    {
        ...
        var y = Math.Clamp(
            ...
            safeArea.Top,
            safeArea.Bottom - (int)panelHeight);
        return new(x, y, (int)panelWidth, (int)panelHeight);
    }
    ...
    private int? GetHoveredEntryIndex(InputState input)
    {
        for (var i = 0; i < menuEntries.Count; i++)
        {
            if (GetMenuEntryHitBounds(menuEntries[i]).Contains(input.MousePosition)) return i;
        }
        return null;
    }
}
