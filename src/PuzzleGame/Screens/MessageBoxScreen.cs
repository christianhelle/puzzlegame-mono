...
public sealed class MessageBoxScreen : GameScreen
{
    ...
    public override void HandleInput(InputState input)
    {
        ...
        {
            if (includeCancel) OnCancel(playerIndex);
            else ExitScreen();
            return;
        }
    ...
    public override void Draw(GameTime gameTime)
    {
        ...
        spriteBatch.Draw(
            ...
            dialogBounds,
            new Color(r: 15, g: 21, b: 32) * (0.7f * TransitionAlpha));
        DrawBorder(spriteBatch, dialogBounds, new Color(r: 110, g: 131, b: 168) * TransitionAlpha);
        var messagePosition = new Vector2(dialogBounds.X + 28f, dialogBounds.Y + 24f);
        spriteBatch.DrawString(
            font,
            wrappedMessage,
            messagePosition + new Vector2(x: 2f, y: 2f),
        ...
        DrawButton(
            ...
            acceptText,
            acceptBounds,
            acceptBounds.Contains(mousePosition));
        if (includeCancel)
            DrawButton(
                spriteBatch,
                font,
                cancelText,
                cancelBounds,
                cancelBounds.Contains(mousePosition));
        spriteBatch.End();
    }
    ...
    private (Rectangle DialogBounds, Rectangle AcceptBounds, Rectangle CancelBounds, string
        WrappedMessage) GetLayout()
    {
        ...
        var buttonWidth = 180;
        var buttonHeight = 54;
        var dialogWidth = (int)MathF.Min(safeArea.Width, MathF.Max(x: 460f, textSize.X + 56f));
        var dialogHeight = (int)MathF.Min(safeArea.Height, MathF.Max(x: 220f, textSize.Y + 128f));
        ...
        var acceptX = includeCancel ? dialogBounds.Center.X - buttonWidth - 10
            : dialogBounds.Center.X - buttonWidth / 2;
        var acceptBounds = new Rectangle(acceptX, buttonsY, buttonWidth, buttonHeight);
        var cancelBounds = includeCancel ? new(
            dialogBounds.Center.X + 10,
            buttonsY,
            buttonWidth,
            buttonHeight) : Rectangle.Empty;
